...
export const FooterAuthorLinksListItem = ({
  link,
  ...properties
}: FooterAuthorLinksListItemProperties): ReactNode => {
  return (
    <Link
      href={link.href}
      {...FOOTER_AUTHORS_LIST_PROPERTIES.AUTHOR_LINKS_ITEM}
      {...properties}
    >
      <link.Icon size={16} />
    </Link>
...
export const FooterAuthorLinksList = ({
  links,
  ...properties
}: FooterAuthorLinksListProperties): ReactNode => {
  return (
    <div {...FOOTER_AUTHORS_LIST_PROPERTIES.AUTHOR_LINKS} {...properties}>
      {links.map((link) => (
        <FooterAuthorLinksListItem key={link.name} link={link} />
      ))}
    </div>
  );
};
  ...
  return Object.keys(authors).map((author) => {
    ...
    return (
      <div key={author} {...FOOTER_AUTHORS_LIST_PROPERTIES.AUTHOR_LINKS_PARENT}>
        <span>© {author}</span>
        <FooterAuthorLinksList links={links} />
      </div>
    );
  });
};

export const FooterAuthorsList = ({
  authors,
  ...properties
}: FooterAuthorsListProperties) => {
